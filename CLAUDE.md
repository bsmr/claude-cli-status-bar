...
- **Runtime contract**: read JSON from stdin, write the rendered status bar to stdout, exit 0. Claude Code renders every line it receives, and the default layout emits two rows — the "single line" of the original 0.1.x contract no longer holds. Errors go to stderr; non-zero exit makes Claude Code fall back silently. Since 0.4.22 the contract has one exception: invoked with no arguments while stdin is a terminal, ccsb prints its usage and exits 0 rather than reading stdin at all.
...
- **Proxy mode** (no args and stdin is not a terminal — the path Claude Code invokes): read stdin once, capture the raw bytes to `$XDG_STATE_HOME/ccsb/captures/<timestamp>-<session_id>.json` (RFC3339Nano with `-` for `:` since 0.4.20, because `:` is illegal on NTFS; the session id is capped at 64 chars so it cannot blow past NAME_MAX), then either forward the payload to a configured proxied statusLine command (its stdout becomes ours) or render a built-in fallback. The rendered output and any proxy stderr are captured alongside the input as `.out` and `.err` files sharing the same basename, so input and result can be paired by readers.
- **Interactive invocation** (no args, stdin is a terminal): print a two-line explanation followed by the full usage, exit 0, and never touch stdin, the config or the state directory. The check is `stdinIsTTY` in `internal/pkg/cli/cli.go`, resolved in `NewFromOS` and carried in `cli.Flags` alongside `NoColor`. Claude Code's `CLAUDECODE` environment variable was measured to be present in the statusLine child but deliberately rejected as the signal: were it ever renamed, ccsb would mistake a real call for an interactive one and write usage text into the status bar, whereas a piped stdin can never be mistaken for a terminal. A deliberate consequence of the character-device test is that `ccsb </dev/null` counts as interactive.
...
- **`ccsb uninstall`**: strict inverse — only proceeds if `statusLine` currently points at this binary. Restores the backup value (or removes the key if there was no prior). The JSON *value* is preserved, including key order; the exact bytes are not, because `json.MarshalIndent` re-indents the stored `json.RawMessage` and Go's encoder escapes `&`, `<` and `>`.
- **`ccsb status`**: prints `hooked: yes/no`, resolved paths, current proxy command, and current backup.
- **`ccsb captures clean [--older-than <duration>]`**: removes capture files. Without arguments the cutoff is `time.Now()`, so everything goes — "remove all" is not a separate code path. `--older-than` accepts a day count (`7d`) or any Go duration (`24h`); `time.ParseDuration` has no day unit, so a trailing `d` is converted to hours in `parseRetention`. Age comes from the timestamp in the basename, never from `stat`; `TimeFromName` accepts both the current layout and the pre-0.4.20 RFC3339Nano one, so an existing capture directory stays prunable. Files without a parsable timestamp and all subdirectories are left alone.
