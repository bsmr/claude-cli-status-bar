...
> **Status:** active development — `0.2.20`. Proxy mode, capture, hook
...
> file next to each broken capture, persistent `schema_version`
> tracking that flags upstream payload-schema bumps, `ccsb mode`
> for proxy/native switching, `ccsb config reset` for restoring
> defaults, and `ccsb doctor` for installation health plus a
> schema-drift diff against the latest capture. `ccsb install`
