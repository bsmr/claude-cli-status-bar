...
> **Status:** active development — `0.2.17`. Proxy mode, capture, hook
...
> parsing (a type error in one field no longer kills the whole
> bar), `ccsb mode` for proxy/native switching, and `ccsb config
> reset` for restoring defaults. `ccsb install` defaults to native
> mode when the previous `statusLine` was the canonical
