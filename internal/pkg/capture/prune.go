...
// TimeFromName parses the RFC3339Nano timestamp basename puts in front of
// the session id. The timestamp is always formatted in UTC, so it ends at
// the first "Z" in the name. Returns false for any name that does not carry
// one — callers treat those as foreign files and leave them alone.
//
// The fractional second is not fixed width (RFC3339Nano trims trailing
// zeros), so names must be compared as time.Time rather than lexically.
